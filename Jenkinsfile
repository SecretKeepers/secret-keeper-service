pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: 'ansible-secret-server', keyFileVariable: 'SSH_KEY')]) {
                        sh "ansible-playbook -i /etc/ansible/hosts --private-key=$SSH_KEY secret-deploy.yml"
                    }
                }
            }
        }
